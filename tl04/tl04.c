...
int stack_push(const char *question, enum category_t category) {
    ...
    if ((question == NULL) || (strlen(question) == 0)) { // failure base cases
        return FAILURE;
    }

    struct post_t *newPost = (struct post_t *) malloc(sizeof(struct post_t)); // allocate space for post
    if (newPost == NULL) {
        free(newPost);
        return FAILURE;
    }

    char *newQuestion = (char *) malloc(strlen(question) + 1); // allocate space for question
    if (newQuestion == NULL) {
        free(newQuestion);
        free(newPost);
        return FAILURE;
    ...
    // create new post
    newPost->question = newQuestion;
    newPost->category = category;

    if (numPosts == capacity) { // max capacity case
        struct post_t **temp = (struct post_t **) realloc(stack_arr, sizeof(struct post_t) * (2 * capacity)); // double capacity
        if (temp == NULL) {
    ...
    stack_arr[numPosts] = newPost; // add to top of the stack
    numPosts++; // update numPosts
    return SUCCESS;
}
...
int stack_pop(struct post_t *data) {
    ...
    if (data == NULL || numPosts == 0) { // failure base cases
        return FAILURE;
    }

    struct post_t *popped = stack_arr[numPosts - 1]; // remove the top of the stack
    *data = *popped; // set data to point to removed post
    free(popped);

    stack_arr[numPosts - 1] = NULL; // set pointer of the popped post to NULL
    numPosts--; // update numPosts
    return SUCCESS;
}
...
int destroy_stack(void) {
    ...
    if (stack_arr == NULL) { // no stack to destroy
        return FAILURE;
    }

    for (int i = 0; i < numPosts; i++) {
        free(stack_arr[i]->question); // free pointer to question
        free(stack_arr[i]); // free element
    }

    free(stack_arr); // free backing array
    numPosts = 0; // update numPosts
