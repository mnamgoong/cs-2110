...
public class Operations
{
    ...
    {
        // (which << 4) determines which short to start at (2^4 intervals)
        // (num >> (which << 4)) readjusts int num to either B0 or B1
        // 0b0b1111111111111111 gives a 16-bit short
    ...
    {
        // (which << 3) determines which byte to start at (2^3 intervals)
        // (num & ~(0b11111111 << (which << 3))) will clear the chosen byte
        // (a_byte << (which << 3)) aligns a_byte with the cleared byte
        // the | inserts a_byte into the cleared byte
    ...
    {
        // (s2 << 16) shifts the short to leave room for 16 bits (2 bytes)
        // (b1 << 8) shifts the byte to leave room for 8 bits (1 byte)
        // the | combines s2, b1, and b0 in that order
    ...
    {
        // XNOR is true if either both are true or neither are true
    ...
    {
        // if the last bit (0b1) is 0, then int num is divisible by 2
        return (num & 0b1) == 0;
    }
    ...
    {
        // if the last 3 bits (0b111) are 0, then int num is divisible by 8
        return (num & 0b111) == 0;
    }
